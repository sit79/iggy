...
async function grabPage(browser, link) {
  ...
  if (!dataObj.alreadyScraped) {
    ...
    dataObj['artists'] = await newPage.evaluate(() => {
      ...
      return Array.from(artistNodeCollection).map((item) => item.innerText)
    })
    ...
    dataObj['trackTitles'] = await newPage.evaluate(() => {
      ...
      return Array.from(titleNodeCollection).map((item) => item.innerText)
    })
  }
...
function readPage(page) {
  ...
  if (page.alreadyScraped && page.published) {
    ...
    spinner
      .warn(
        chalk.yellow(
          chalk.bold(`${page.showTitle}`) + ` => scraped but not yet published.`
        )
      )
      .stop()
  } else {
