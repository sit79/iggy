...
import { createDate, removeReadMore, _ora } from "./helper.js";
...
const scrape = async () => {
    ...
    spinner = _ora;
    spinner.start('collecting pages');
    for (let link of urls) {
        let dataObj = await grabPage(browser, link);
        pages.push(dataObj)
    };
    spinner.succeed(chalk.green('pages collected\n'));
...
async function grabPage(browser, link) {
    let dataObj = {};
    ...
    dataObj["published"] = fs.existsSync(`${path.join(FILEPATH, "published", dataObj["fileName"])}.txt`);
    dataObj["alreadyScraped"] = dataObj["published"] || fs.existsSync(`${dataObj["path"]}.txt`);
...
function readPage(page) {
    ...
    if (page.alreadyScraped && page.published) {
        spinner.succeed(chalk.bold(`${page.showTitle}`) + ` => already published.`).stop();
    } else if (page.alreadyScraped && !page.published) {
        spinner.warn(chalk.yellow(chalk.bold(`${page.showTitle}`) + ` => scraped but not yet published.`)).stop();
    } else {
        ...
        for (let i = 0, k = page.artists.length; i < k; i++) {
            const artistAndTack = `${i + 1}. ${page.artists[i]} – ${page.trackTitles[i]
                } \n`;
            showResult += artistAndTack;
        }
        fs.writeFileSync(`${page.path}.txt`, showResult, { encoding: "utf-8" });
        spinner.succeed(chalk.green(chalk.bold(`${page.showTitle}`) + ` => ready to be fetched.`)).stop();
    }
}
